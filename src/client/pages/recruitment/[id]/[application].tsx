...
import {Badge, Card, Col, ListGroup, Media, Nav, ProgressBar, Row, Tab, Table, Tabs} from "react-bootstrap";
...
export const getServerSideProps = withAuthServerSideProps(async (context) => {
	try {
		const applicationResponse = await api.getRecruitmentApplication(context.req.query.application);
...
const RecruitmentApplications = ({user, data}) => {
	...
	useEffect(() => {
		console.log(data);
		if (!data.application) {
			router.push({pathname: '/recruitment'});
		}
	})
	...
	return (
		<Row>
			<Col>
				<Card>
					<Card.Body>
						<Card.Title>Podanie na {data.application.position.role.name}</Card.Title>
						...
						<Tabs defaultActiveKey="general">
							...
							<Tab eventKey="steam" title="Steam">
								...
								<Row>
									<Col>
										<img
											...
											height={64}
											className="mr-3"
											src={data.application.author.steam_profile.avatar_medium}
											alt="Generic placeholder"
										/>
									</Col>
								</Row>
								<Row>
									<Col>
										<p>Nazwa uzytkownika: <Badge
											variant={"secondary"}>{data.application.author.steam_profile.nickname}</Badge>
										</p>
									</Col>
								</Row>
								<Row>
									<Col>
										<p>SteamID32: <Badge
											variant={"secondary"}>{data.application.author.steam_profile.steamid32}</Badge>
										</p>
									</Col>
								</Row>
								<Row>
									<Col>
										<p>SteamID64: <Badge
											variant={"secondary"}>{data.application.author.steam_profile.steamid64}</Badge>
										</p>
									</Col>
								</Row>
								<Row>
									<Col>
										<p>Profil: <a href={`${data.application.author.steam_profile.url}`}>{data.application.author.steam_profile.url} </a>
										</p>
									</Col>
								</Row>
							</Tab>
							<Tab eventKey="steamrep" title="SteamRep">
								...
								<Row>
									<Col>
										<iframe src={`https://steamrep.com/profiles/${data.application.author.steam_profile.steamid64}`} width={1000} height={500}/>
									</Col>
								</Row>
							</Tab>
						</Tabs>
