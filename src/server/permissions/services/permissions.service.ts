...
export class PermissionsService implements OnModuleInit {
  ...
  async remove(entity: Permission, options?: RemoveOptions): Promise<any> {
    return options
      ? this.permissionRepository.remove(entity, options)
      : this.permissionRepository.remove(entity);
  }
    ...
    try {
      ...
      for await (const module of modules) {
        ...
        for await (const permission of permissions) {
          if (
            !(await this.findOne({
              where: { module: moduleName, value: permission },
            }))
          ) {
            const newPerm = await this.create({
              module: moduleName,
              value: permission,
            });
            this.logger.log(
              `[${moduleName}] dodano uprawnienie <${newPerm.value}>`,
            );
          }
        }
      }
