...
export class UsersService extends TypeOrmCrudService<User> {
  ...
  async update(user: User, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    user.steam_profile =
      updateUserDto.steam_profile !== undefined
        ? updateUserDto.steam_profile
        : user.steam_profile;
    user.is_active =
  ...
  async register(
    ...
    const userRole = await this.rolesService.findOne({
      where: { name: 'Uzytkowni' },
    });
  ...
  async changePassword(
    ...
    if (newPasswordNotEqual)
      throw new BadRequestException(
        'New password and new password equal not equal',
      );
  ...
  async sendChangeEmail(
    userId: number,
    req: Request,
    changeEmailDto: SendChangeEmailEmailDto,
  ) {
