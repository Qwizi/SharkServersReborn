...
export class RolesService implements OnModuleInit {
  ...
  async remove(entity: Role, options?: RemoveOptions): Promise<any> {
    return options
      ? this.rolesRepository.remove(entity, options)
      : this.rolesRepository.remove(entity);
  }
  ...
  async hasPerm(role: Role, permModule: string) {
    try {
      ...
      return !!role.permissions.find(
        (perm) => perm.module == module && perm.value == value,
      );
    } catch (e) {
      this.logger.error(e.message);
    }
  ...
  async createDefaultRoles() {
    ...
    const usersPermissions = await this.permissionsService.find({
      where: [
        ...
        {
          module: 'recruitment',
          value: In([Perms.CREATE_APPLICATION, Perms.CREATE_COMMENT]),
        },
      ],
    });
    const adminRole = {
