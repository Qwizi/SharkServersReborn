...
export class SteamProfileService implements OnModuleInit {
  ...
  constructor(
    ...
    private steamProfileRepository: Repository<SteamProfile>,
  ) {
  ...
  async _create(
    createSteamProfileDto: CreateSteamProfileDto,
  ): Promise<SteamProfile> {
    const steamProfile = await this.steamProfileRepository.create(
      createSteamProfileDto,
    );
  ...
  async create(
    createSteamProfileWithSteamID64OnlyDto: CreateSteamProfileWithSteamID64OnlyDto,
  ): Promise<SteamProfile> {
  ...
  async findOne(
    options?: FindOneOptions<SteamProfile>,
  ...
  async update(
    ...
    updateSteamProfileDto: UpdateSteamProfileDto,
  ): Promise<SteamProfile> {
    steamProfile.nickname =
      updateSteamProfileDto.nickname || steamProfile.nickname;
    steamProfile.steamid64 =
      updateSteamProfileDto.steamid64 || steamProfile.steamid64;
    steamProfile.steamid32 =
      updateSteamProfileDto.steamid32 || steamProfile.steamid32;
    steamProfile.avatar = updateSteamProfileDto.avatar || steamProfile.avatar;
    steamProfile.avatar_medium =
      updateSteamProfileDto.avatar_medium || steamProfile.avatar_medium;
    steamProfile.avatar_full =
      updateSteamProfileDto.avatar_full || steamProfile.avatar_full;
  ...
  async remove(entity: SteamProfile, options?: RemoveOptions): Promise<any> {
    return options
      ? this.steamProfileRepository.remove(entity, options)
      : this.steamProfileRepository.remove(entity);
  }
}
