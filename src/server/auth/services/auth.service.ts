...
export class AuthService {
  ...
  async validateUser(username: string, password: string): Promise<any> {
    const user = await this.usersService.findOne({
      ...
      relations: ['roles', 'roles.permissions', 'steam_profile'],
    });
    if (
      user &&
  ...
  async connectSteamAccount(user, profile) {
    ...
    console.log(steamid);
    const steamProfileExist = await this.steamProfileService.findOne({
      where: { steamid64: steamid },
    });
    console.log(steamProfileExist);
    if (steamProfileExist) {
      await this.usersService.update(user, {
        steam_profile: steamProfileExist,
      });
    } else {
      const newSteamProfile = await this.steamProfileService.create({
        steamid64: steamid,
      });
  ...
  async disconnectAccount(
    user: User,
    disconnectAccountDto: DisconnectAccountDto,
  ) {
