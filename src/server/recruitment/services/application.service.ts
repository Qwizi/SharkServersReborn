...
export class ApplicationService
  extends TypeOrmCrudService<Application>
  implements OnModuleInit
{
  ...
  async create(
    ...
    if (position.free_space === 0)
      throw new BadRequestException('Position free space is equal 0');
    ...
    const questionsNotFound = questionsId.filter((e) =>
      questionsFromDb.includes(e),
    );
    ...
    for await (const item of answers) {
      ...
      const questionAnswer = await this.positionQuestionAnswerService.createOne(
        crudReq,
        {
  ...
  async runAction(
    ...
    if (application.status === ApplicationStatus.ACCEPTED) {
      ...
      if (
        !applicationAuthorRoles.find(
          (role) => role.id === applicationPositionRole.id,
        )
      ) {
        await this.usersService.update(application.author, {
          roles: [...application.author.roles, applicationPositionRole],
        });
      }
