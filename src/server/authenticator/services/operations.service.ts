...
export class OperationsService implements OnModuleInit {
  ...
  constructor(
    ...
    private operationsService: Repository<Operation>,
  ) {}
  ...
  async findOne(
    options?: FindOneOptions<Operation>,
  ...
  async update(
    ...
    operation.is_active =
      updateOperationDto.is_active !== undefined
        ? updateOperationDto.is_active
        : updateOperationDto.is_active;
    await this.operationsService.save(operation);
    return operation;
  }
  ...
  async remove(entity: Operation, options?: RemoveOptions): Promise<any> {
    return options
      ? this.operationsService.remove(entity, options)
      : this.operationsService.remove(entity);
  }
