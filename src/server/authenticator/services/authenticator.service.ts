...
export class AuthenticatorService implements OnModuleInit {
  ...
  async checkCode(
    ...
    type?: Operations,
  ): Promise<[boolean, Operation | undefined]> {
    const key = `code:${code}`;
    const operation: Operation = await this.cacheManager.get<Operation>(key);
    const expression = type
      ? operation && operation.is_active && operation.type === type
  ...
  async filterOperations(
    ...
    type: Operations = Operations.CONFIRM_EMAIL,
    isActive: boolean = false,
  ) {
    return operations.filter(
      (op) => op.is_active !== isActive && op.type === type,
    );
  }

  async deactivateConfirmCodes(
    user: User,
    type: Operations = Operations.CONFIRM_EMAIL,
    isActive: boolean = false,
  ) {
    if (user.operations || user.operations.length > 0) {
      const filteredOperations = await this.filterOperations(
        user.operations,
        type,
        isActive,
      );
